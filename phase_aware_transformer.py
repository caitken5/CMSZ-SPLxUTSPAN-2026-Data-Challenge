...
K = 5  # GMM components (will map to 4 phases)
EPOCHS = 200  # training
EARLY_PATIENCE = 20
...
# Phase-aware config
USE_PHASE_AWARE = True  # NEW: Enable phase-aware processing
N_PHASES = 4  # Pre-prep, Preparation, Shot, Follow-through

SAVE_FINAL = "final_phase_aware_transformer.pt"
...
# %% NEW: Phase Extraction Functions
def extract_phase_boundaries(gmm_labels: np.ndarray, n_phases: int = 4) -> list:
    """
    Convert GMM labels (0-4) to phase boundaries.
    
    Strategy: Map GMM components to phases based on temporal ordering
    Pre-prep -> Preparation -> Shot -> Follow-through
    
    Returns: List of (start_idx, end_idx) for each phase
    """
    T = len(gmm_labels)
    
    # Find when each GMM component dominates
    phase_starts = []
    current_label = gmm_labels[0]
    phase_starts.append(0)
    
    for t in range(1, T):
        if gmm_labels[t] != current_label:
            phase_starts.append(t)
            current_label = gmm_labels[t]
    
    # If we have more transitions than needed, merge smaller phases
    # If fewer, split largest phase
    if len(phase_starts) < n_phases:
        # Split the longest phase
        phase_lengths = []
...
def segment_shot_into_phases(shot: np.ndarray, gmm, scaler, n_phases: int = 4):
    """
    Segment a single shot into phases using GMM.
    
    Returns:
    - phases: list of (T_phase, n_signals) arrays
    - boundaries: list of (start, end) tuples
    """
    # Get GMM labels for this shot
    X_shot = scaler.transform(shot)
    labels = gmm.predict(X_shot)
    
    # Extract phase boundaries
    boundaries = extract_phase_boundaries(labels, n_phases)
    
    # Extract phase subsequences
    phases = []
    for start, end in boundaries:
        if end > start:  # Valid phase
            phases.append(shot[start:end, :])
        else:  # Empty phase (shouldn't happen but handle it)
            # Use a single-frame placeholder
...
class PhaseEncoder(nn.Module):
    """Encoder for a single phase"""
    def __init__(self, d_in, d_model=128, nhead=8, num_layers=2, dropout=0.1, max_len=400):
        super().__init__()
        
        self.conv = nn.Sequential(
            nn.Conv1d(in_channels=d_in, out_channels=d_model, kernel_size=7, padding=3),
...
class PhaseAwareTransformer(nn.Module):
    """
    Phase-aware shot transformer.
    Processes each phase separately, then combines for prediction.
    """
    def __init__(
        self, d_in, d_model=128, nhead=8, num_layers=3, dropout=0.1, 
        max_len=400, n_phases=4
    ):
        super().__init__()
        self.n_phases = n_phases
        
        # Separate encoder for each phase
        self.phase_encoders = nn.ModuleList([
            PhaseEncoder(d_in, d_model, nhead, num_layers//2, dropout, max_len)
            for _ in range(n_phases)
        ])
        
        # Learnable phase importance weights
        self.phase_weights = nn.Parameter(torch.ones(n_phases))
        
        # Fusion layer
        fusion_dim = d_model * n_phases
    ...
    def forward(self, phase_inputs, phase_masks):
        """
        phase_inputs: list of n_phases tensors, each (B, T_phase, d_in)
        phase_masks: list of n_phases tensors, each (B, T_phase) 
        """
        # Encode each phase
        phase_embeddings = []
        for i, (x, mask) in enumerate(zip(phase_inputs, phase_masks)):
            emb = self.phase_encoders[i](x, mask)  # (B, d_model)
            # Apply learned phase weight
            emb = emb * torch.sigmoid(self.phase_weights[i])
            phase_embeddings.append(emb)
        
        # Concatenate all phase embeddings
        combined = torch.cat(phase_embeddings, dim=1)  # (B, d_model * n_phases)
...
class PhaseShotDataset(Dataset):
    """Dataset that returns phase-segmented shots"""
    def __init__(self, phase_shots_list, y_df):
        """
        phase_shots_list: list of lists, where each inner list has n_phases arrays
        y_df: DataFrame with angle, depth, left_right
        """
        self.phase_shots = phase_shots_list
    ...
    def __len__(self):
        return len(self.phase_shots)

    def __getitem__(self, idx):
        return self.phase_shots[idx], self.y_angle[idx], self.y_depth[idx], self.y_lr[idx]


def collate_phase_pad(batch):
    """Collate function for phase-aware batches"""
    phase_shots_batch, angles, depths, lrs = zip(*batch)
    
    B = len(phase_shots_batch)
    n_phases = len(phase_shots_batch[0])
    d = phase_shots_batch[0][0].shape[1]
    
    # Find max length for each phase
    max_lens = []
    for p in range(n_phases):
        max_lens.append(max(phase_shots_batch[i][p].shape[0] for i in range(B)))
    ...
    for p in range(n_phases):
        maxT = max_lens[p]
        x_pad = torch.zeros(B, maxT, d, dtype=torch.float32)
        mask = torch.zeros(B, maxT, dtype=torch.bool)
        
        for i in range(B):
            phase_data = phase_shots_batch[i][p]
            T = phase_data.shape[0]
            x_pad[i, :T, :] = torch.from_numpy(phase_data.astype(np.float32))
            mask[i, :T] = True
        
        phase_inputs.append(x_pad)
...
def collate_phase_pad_infer(batch):
    """Collate for inference (no labels)"""
    phase_shots_batch = [b[0] for b in batch]
    ...
    return phase_inputs, phase_masks
...
def build_phase_features(
    ...
    """
    Build phase-segmented features.
    
    Returns: list of lists, where each inner list has n_phases feature arrays
    """
    phase_shots_all = []
    
    for shot in shots_list:
        # Segment into phases
        phases, boundaries = segment_shot_into_phases(shot, gmm_, scaler_, n_phases)
        
        # Build features for each phase
        phase_features = build_features(
            phases,
            signal_cols_,
            scaler_,
            gmm_,
        ...
        phase_shots_all.append(phase_features)
    
    return phase_shots_all
...
def evaluate_mae(model, loader):
    ...
    with torch.no_grad():
        for phase_inputs, phase_masks, a, d, lr in loader:
            phase_inputs = [x.to(DEVICE) for x in phase_inputs]
            phase_masks = [m.to(DEVICE) for m in phase_masks]
            a, d, lr = a.to(DEVICE), d.to(DEVICE), lr.to(DEVICE)
            pa, pdd, plr = model(phase_inputs, phase_masks)
...
def run_fold(fold_id, train_idx, val_idx):
    ...
    X_train_phase = build_phase_features(
        shots_train,
        signal_cols,
        scaler_fold,
        gmm_fold,
    ...
    X_val_phase = build_phase_features(
        shots_val,
        signal_cols,
        scaler_fold,
        gmm_fold,
    ...
    d_in_fold = X_train_phase[0][0].shape[1]

    train_loader = DataLoader(
        PhaseShotDataset(X_train_phase, y_train),
        batch_size=64,
        shuffle=True,
        collate_fn=collate_phase_pad,
    )
    val_loader = DataLoader(
        PhaseShotDataset(X_val_phase, y_val),
        batch_size=64,
        shuffle=False,
        collate_fn=collate_phase_pad,
    )

    model = PhaseAwareTransformer(
        d_in=d_in_fold,
        d_model=D_MODEL,
        nhead=NHEAD,
    ...
    early = EarlyStopping(
        ...
        mode="min",
        save_path=f"best_phase_model_fold{fold_id}.pt",
    )

    for ep in range(1, EPOCHS + 1):
        model.train()
        for phase_inputs, phase_masks, a, d, lr in train_loader:
            phase_inputs = [x.to(DEVICE) for x in phase_inputs]
            phase_masks = [m.to(DEVICE) for m in phase_masks]
            a, d, lr = a.to(DEVICE), d.to(DEVICE), lr.to(DEVICE)
    ...
    model.load_state_dict(
        torch.load(f"best_phase_model_fold{fold_id}.pt", map_location=DEVICE)
    )
...
d_in = X_train_phase[0][0].shape[1]
...
# Build PHASE features using FINAL scaler+gmm
test_phase_features = build_phase_features(
    test_shots,
    signal_cols_test,
    scaler,
    gmm,
...
test_loader = DataLoader(
    [(x,) for x in test_phase_features],
    batch_size=64,
    shuffle=False,
    collate_fn=collate_phase_pad_infer,
)

model.eval()
pred_angle, pred_depth, pred_lr = [], [], []
with torch.no_grad():
    for phase_inputs, phase_masks in test_loader:
        phase_inputs = [x.to(DEVICE) for x in phase_inputs]
        phase_masks = [m.to(DEVICE) for m in phase_masks]
        a, d, lr = model(phase_inputs, phase_masks)
...
submission.to_csv("submission_phase_aware.csv", index=False)
print("Wrote submission_phase_aware.csv")
